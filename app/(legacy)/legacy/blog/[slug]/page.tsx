...
export default async function BlogPost({ params }: { params: { slug: string } }) {
  ...
  // Read the MDX file from the archived legacy content directory.
  const postPath = path.join(process.cwd(), 'app/(legacy)/legacy/_content/blog', `${slug}.mdx`);
  ...
  const components = {
    ...
    ul: (props: any) => (
      <ul className="list-disc pl-6 mb-6 font-body" {...props} />
    ),
    ol: (props: any) => (
      <ol className="list-decimal pl-6 mb-6 font-body" {...props} />
    ),
    li: (props: any) => (
